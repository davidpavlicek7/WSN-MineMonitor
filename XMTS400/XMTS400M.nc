...
implementation
{
  ...
  enum {SENSOR_NONE = 0,
	...
	TEMP_THRESHOLD = 27; //our
	LIGHT_THRESHOLD = 0.8; //our ????
	HUMIDITY_WARNING_THRESHOLD1 = 30; //28-11-2014
	HUMIDITY_WARNING_THRESHOLD2 = 34; //28-11-2014
 ...
 task void stopTempHumControl(){
  ...
  task void stopTaosControl(){
   ...
   task void powerOffAccel(){
	...
	//our
	task void checkTempLimits(){
		temp_data = -38.4 + 0.0098 * temp_data;
		if (temp_data < TEMP_THRESHOLD){
			//send temp data packet
			call Leds.redToggle();				//28-11-2014
		}else{
			call Leds.greenToggle();			//28-11-2014
		}
		return;
	}
	
	//our
	task void checkHumLimits(){
		if(hum_data < HUMIDITY_WARNING_THRESHOLD1){
			call Leds.greenToggle();							//28-11-2014
		}else if(hum_data > HUMIDITY_DANGER_THRESHOLD1){
			// send CO2 levels higher warning packet (CO2 is represented by humidity)
			//(CO2 lvls have risen above normal lvls but not dangerous yet)
			call (Leds.yellowToggle() && Leds.greenToggle());	//28-11-2014
		}else if(hum_data > HUMIDITY_DANGER_THRESHOLD2){		//28-11-2014
			// send dangerous CO2 levels packet (this area is dangerous to humans)
			call (Leds.yellowToggle() && Leds.redToggle());		//28-11-2014
		}
		return;
	}
    
	task void checkLightLimit(){
 ...
 *****************************************************************************/
  ...
  async event result_t Humidity.dataReady(uint16_t data) {
	...
	hum_data = data;		//our	28-11-2014
	post checkHumLimits();	//our	28-11-2014
